...
VSOP87_L_J2000 = [
    ...
    # L1
    [[628307584999., 0.0, 0.0], [206059., 2.678235, 6283.07585],
     [4303., 2.6351, 12566.1517], [425., 1.59, 3.523], [119., 5.796, 26.298],
     [109., 2.966, 1577.344], [93., 2.59, 18849.23], [72., 1.14, 529.69],
     [68., 1.87, 398.15], [67., 4.41, 5507.55], [59., 2.89, 5223.69],
     [56., 2.17, 155.42], [45., 0.4, 796.3], [36., 0.47, 775.52],
    ...
    # L2
    [[8722., 1.0725, 6283.0758], [991., 3.1416, 0.0], [295., 0.437, 12566.152],
     [27., 0.05, 3.52], [16., 5.19, 26.3], [16., 3.69, 155.42],
     [9., 0.3, 18849.23], [9., 2.06, 77713.77], [7., 0.83, 775.52],
     [5., 4.66, 1577.34], [4., 1.03, 7.11], [4., 3.44, 5573.14],
     [3., 5.14, 769.3], [3., 6.05, 5507.55], [3., 1.19, 242.73],
     [3., 6.12, 529.69], [3., 0.3, 398.15], [3., 2.28, 553.57],
     [2., 4.38, 5223.69], [2., 3.75, 0.98]
     ],
    # L3
    [[289., 5.842, 6283.076], [21., 6.05, 12556.15], [3., 5.2, 155.42],
     [3., 3.14, 0.0], [1., 4.72, 3.52], [1., 5.97, 242.73],
     [1.0, 5.54, 18849.23]
     ],
    # L4
    [[8., 4.14, 6283.08], [1., 3.28, 12566.15]
     ]]
"""This table contains Earth's most important periodic terms from the planetary
...
VSOP87_B_J2000 = [
    ...
    # B1
    [[227778., 3.413766, 6283.07585], [3806., 3.3706, 12566.1517],
     [3620., 0.0, 0.0], [72., 3.33, 18849.23], [8., 3.89, 5507.55],
     [8., 1.79, 5223.69], [6., 5.2, 2352.87]
     ],
    # B2
    [[9721., 5.1519, 6283.07585], [233., 3.1416, 0.0],
     [134., 0.644, 12566.152], [7., 1.07, 18849.23]
     ],
    # B3
    [[276., 0.595, 6283.076], [17., 3.14, 0.0], [4., 0.12, 12566.15]
     ],
    # B4
    [[6., 2.27, 6283.08], [1., 0.0, 0.0]
     ]]
"""This table contains Earth's most important periodic terms from the planetary
...
class Earth(object):
    ...
    def geometric_heliocentric_position_J2000(epoch, toFK5=True):
        ...
        if not isinstance(epoch, Epoch):
            raise TypeError("Invalid input types")
        # Second, call auxiliary function in charge of computations
        return geometric_vsop_pos(epoch, VSOP87_L_J2000, VSOP87_B_J2000,
                                  VSOP87_R, toFK5)
